              ...
              <li v-if="settings.currency_switcher != 0">
                <div class="dropdown">
                  <button class="dropdown-toggle" type="button" id="" @click.stop="currencyDropdown"
                          :class="{ show: currency_dropdown }" data-bs-toggle="dropdown" aria-expanded="false">
                    {{ activeCurrency.name }}
                  </button>
        ...
        <div class="botom-content">
          <div class="sg-logo">
                ...
                <button class="sg-toggle" @click="toggleCategory">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 18">
                    <g id="list" transform="translate(0 -3)">
                      <path id="Path_4186" data-name="Path 4186" d="M7,6H23a1,1,0,0,0,0-2H7A1,1,0,0,0,7,6Z"
                            fill="#fff"/>
            ...
            <router-link :to="{ name: 'home' }">
              ...
              <img v-else :src="settings.dark_logo" alt="Logo" class="img-fluid" width="138"/>
            </router-link>
          </div>
          <div class="sg-search">
                    ...
                    <li v-for="(cart, index) in carts" :key="index">
                      <div class="sg-product" v-if="!cart.is_buy_now">
                        ...
                        <div class="product-thumb">
                          <router-link :to="{ name: 'product.details', params: { slug: cart.product_slug } }"><img
                              loading="lazy" :src="cart.image_40x40" :alt="cart.product_name" class="img-fluid"/>
                          </router-link>
                        </div>
        ...
        <ul @click.stop class="global-list">
          <li class="nav-item" v-for="(menu, i) in headerMenu" :key="'menu' + i"
            ...
            <router-link v-else :to="menu.url" :class="{ active: menu.url == $route.fullPath }"> {{ menu.label }}<span
                v-if="Object.keys(menu).length > 2" class="icon mdi mdi-name mdi-chevron-down"></span></router-link>
            <ul class="sg-dropdown-menu"
                :class="{ 'd-block': home_child_id == menu.id, 'd-none': home_child_id != menu.id }"
                v-if="Object.keys(menu).length > 2">
              <li v-for="(sub_menu, j) in menu" :key="'sub_menu' + j" v-if="sub_menu.url" @click="$store.commit('setSmHomeMenu', false)">
                <a v-if="urlCheck(sub_menu.url)" :href="sub_menu.url">{{ sub_menu.label }}</a>
                <router-link v-else :to="sub_menu.url">{{ sub_menu.label }}</router-link>
              </li>
            </ul>
...
export default {
  ...
  methods: {
    ...
    categoryMenu() {
      ...
      this.$store.commit('setSmHomeMenu', false);
    ...
    homeMenu() {
      this.$store.commit('setSmHomeMenu', !this.smHomeMenu);
    ...
    topBanner() {
      ...
      localStorage.setItem("top-banner-remover", this.authUser.id);
    },
    toggleCategory() {
